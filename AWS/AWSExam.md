...
[오답]
인터넷에서 데이터를 수신합니다.
인터넷에서 Amazon S3으로 "들어오는" 데이터 전송에는 요금이 부과되지 않습니다. S3에서 인터넷으로 "송신" 되는 데이터 전송에 대해서만 비용을 지불합니다.
...
[오답]
데이터베이스 인스턴스 유형 및 크기를 최적화할 필요가 없습니다.
...
190. 다음 중 Amazon S3에서 실수로 데이터가 삭제되지 않도록 보호하는 가장 좋은 방법은 무엇입니까?
- Amazon S3 버전 관리
Amazon S3의 버전 관리는 동일 버킷 내에 여러 개의 객체 변형을 보유하는 수단입니다. S3 버전곤리를 사용하면 버킷에 저장된 모든 버전의 객체를 모두 보존, 검색 및 복원할 수 있습니다. 또한 의도치 않은 사용자 작업 및 앱 장애로부터 더 쉽게 복구할 수 있습니다.

[오답]
Amazon S3 Transfer Acceleration (Amazon S3TA) : S3 버킷 사이에 통신을 빠르게 전송. CloudFront의 엣지 로케이션을 활용한다.
S3 수명 주기 구성 : 비용 효율적. S3 객체를 저렴한 스토리지 클래스로 전환, 보관 또는 삭제 지시함 (비용)

191. "기술 지원 사례를 무제한"으로 "열 수 있는" 최저 support plan은 무엇입니까?
- Developer support plan

192. AWS에서 저렴한 비용으로 자신만의 정적 웹 사이트를 호스팅하고 싶습니다. 이 목적을 위해서는 어떤 서비스를 사용해야 합니까?
"자신만의 정적 웹 사이트"
- Amazon S3 Standard

[오답]
Amazon S3 Infrequent Access : 웹사이트이기 때문에 여러 사용자가 방문할 것..

193. 회사는 방대한 지식 기반에 존재하는 오래된 특허 및 문서(예: PDF, 텍스트 파일, Word 문서 등)에서 사용할 수 있는 정보에 액세스해야 합니다. 회사는 이러한 "지식 리소스를 조사"하고 가장 "관련성이 높은 파일/문서를 반환"할 수 있는 강력한 "검색 도구"를 찾고 있습니다. 다음 중 이 요구 사항을 해결하는 올바른 서비스는 무엇입니까?
- Amazon Kendra
기계학습을 이용한 검색 서비스.
앱에 검색 기능 추가.
 여기에는 Amazon Simple Storage Service(S3), Microsoft SharePoint, Salesforce, ServiceNow, RDS 데이터베이스, 또는 Microsoft OneDrive 등의 다양한 시스템에서 찾을 수 있는 설명서, 연구 보고서, FAQ, 인사(HR) 문서 및 고객 서비스 가이드의 데이터가 포함됩니다.

194. 중단 없는 작업을 수행하기 위해 "3개월" 동안 EC2 인스턴스를 실행하려는 경우 선택할 수 있는 가장 좋은 인스턴스 구매 옵션 유형은 무엇입니까?
- 온디멘드 인스턴스

[오답]
예약 인스턴스 : 최소 1년

195. "인메모리 데이터베이스"로 사용할 수 있는 AWS 서비스는 무엇입니까
- Amazon ElastiCache (인메모리 데이터베이스)

[오답]
RDS : 인메모리 데이터베이스로 사용할 수 없습니다.

196. Non-Serverless 컴퓨팅 서비스에서 Serverless로 전환할 때 얻을 수 있는 주요 이점은 무엇입니까?
- 서버리스는 관리 오버헤드를 제거하므로 앱에 집중할 수 있습니다.

서버리스는 더 많은 운영 책임을 AWS로 이전하여 민첩성과 혁신을 높일 수 있는 클라우드의 기본 아키텍처입니다. 서버리스를 사용하면 서버에 대해 생각하지 않고도 애플리케이션과 서비스를 구축하고 실행할 수 있습니다. 서버 또는 클러스터 프로비저닝, 패치 적용, 운영 체제 유지 관리, 용량 프로비저닝과 같은 인프라 관리 작업이 필요하지 않습니다.

서버리스 애플리케이션을 구축한다는 것은 개발자가 클라우드나 온프레미스에서 서버나 런타임을 관리하고 운영하는 것에 대해 걱정하는 대신 핵심 제품에 집중할 수 있음을 의미합니다. 이러한 오버헤드 감소를 통해 개발자는 확장 가능하고 신뢰할 수 있는 훌륭한 제품을 개발하는 데 투자할 수 있는 시간과 에너지를 회수할 수 있습니다.

[오답]
서버리스가 아닌 서비스를 사용하는 것보다 전반적으로 더 저렴한 비용을 얻을 수 있습니다. : 서버리스를 사용한다고 해서 항상 더 저렴한 비용이 보장되는 것은 아닙니다. 처리 기간이 길고 메모리 사용량이 많은 워크로드와 같이 서버리스 설정 비용이 더 많이 드는 경우가 있을 수 있습니다.

197. 아래 옵션 중 어떤 것을 사용하여 "VPC에 새로운 Amazon RDS 데이터베이스 클러스터를 시작"할 수 있습니까? (2개 선택)
- AWS CloudFormation
- AWS ManageMent Console

Management Console, CLI, SKD, CloudFormation을 사용하여 새로운 RDS 데이터베이스 클러스터를 시작할 수 있습니다.

198. 다음 중 Amazon Route 53이 제공하는 기능은 무엇입니까? (2개 선택)
- 도메인 등록
- DNS 확인

트래픽 흐름

사용이 간편하고 비용 효율적인 글로벌 트래픽 관리 기능: 최종 사용자를 지리적 근접성, 지연 시간, 상태 및 기타 고려 사항을 기반으로 애플리케이션에 최적인 엔드포인트로 라우팅합니다.

[오답]
웹 트래픽 필터링 : WAF의 기능임

199. 배포 단계(개발, 준비, 생산)에 따라 "EC2 인스턴스에 레이블을 지정하고 정렬"할 수 있는 기능은 무엇입니까?
- 인스턴스 태그

200. 다음 중 JSON 문서를 저장하는 데 사용할 수 있는 AWS의 완전 관리형 데이터베이스는 무엇입니까?
- Amazon DynamoDB

Amazon DynamoDB는 어떤 규모에서도 한 자릿수 밀리초의 성능을 제공하는 키-값 및 문서 데이터베이스입니다. 
JSON 형식의 문서에 사용하기에 완벽한 데이터베이스입니다.

[오답]
Amazon Aurora : Amazon Aurora는 MySQL 및 PostgreSQL 호환,관계형 데이터베이스입니다.

201. 고객은 "인터넷 기반 연결보다 더 일관된 네트워크 경험"을 제공하는 "온프레미스 네트워크와 AWS VPC 간에 전용 연결"을 설정해야 합니다. 다음 중 어떤 네트워크 서비스를 사용해야 합니까?
- AWS Direct Connect
...
[오답]
VPN Connection
: VPN은 전용 연결을 제공하는 Direct Connect와 달리 인터넷 기반 연결이기 때문입니다. 인터넷 기반 연결은 VPC에서 온프레미스 네트워크로의 트래픽이 퍼블릭 인터넷을 통과하므로 속도가 느린 것을 의미합니다.

202. 다음 중 AWS Developer support plan에 대한 올바른 설명은 무엇입니까?
- 한 명의 연락처가 무제한 케이스를 열 수 있도록 허용합니다.

203. 회사는 자주 액세스하는 데이터를 Amazon S3에 저장해야 합니다. AWS는 S3 버킷에 객체를 저장하는 데 대해 어떻게 비용을 청구합니까?
- GB당

204. 회사가 AWS에서 간단한 WordPress 사이트를 호스팅할 계획입니다. 그러나 AWS에 대한 개발자의 제한된 지식으로 인해 그들은 사이트를 배포하는 가장 쉬운 방법을 찾고 있습니다.
회사에서는 어떤 AWS 서비스를 사용해야 합니까?
- Amazon Lightsail
가상 프라이빗 서버 솔류션이 필요한 사용자를 위한 PaaS솔루션. 
Lightsail은 개발자에게 클라우드에서 웹 사이트와 웹 어플을 배포하고 관리할 수 있는 컴퓨팅, 스토리지, 네트워킹 용량과 기능 공.
프로젝트를 빠르게 시작하는 데 필요한 모든 것이 있음.

[오답]
Amazon Elastic Beanstalk : Elastic Beanstalk는 실제로 애플리케이션을 배포하고 실행하는 데 사용할 수 있는 PaaS(Platform as a Service)입니다. 그러나 이는 반드시 간단한 것은 아니며, 특히 기본적인 AWS 지식만 있는 회사의 경우 더욱 그렇습니다. 기본 인프라 관리의 일부를 추상화하기는 하지만 여전히 AWS의 인프라 관리에 대한 지식이 필요합니다. 반면에 Lightsail은 사용하기 더 쉽습니다.

->오답은 아는데, 지식이 별로 없으면 lightsail을 이용하세요.

205. 고가용성 아키텍처를 생성하기 위해  Application Load Balancer에 설정해야 하는 최소 가용 영역 수는 몇 개입니까?
- 2
ELB에 최소한 VPC에서2개 이상 가용 영역을 선택하라.

206. RDS에 대한 고객 책임
- 데이터베이스 암호화

207. 활동이 많은 웹 사이트에 Elastic Load Balancing를 사용해야 하는 주된 이유는 무엇입니까?
- ELB는 트래픽을 백엔드 대상에 균등하게 분배하여 수신 트래픽 로드를 처리할 수 있습니다.

208. 회사는 브라우저에 ALB(Application Load Balancer)의 도메인 이름을 입력할 때마다 ALB 뒤에 있는 EC2 웹 서버 중 하나에 연결할 수 없다는 사실을 발견했습니다. 문제에 대해 더 많은 통찰력을 얻으려면 다음 중 무엇을 먼저 확인해야 합니까?
- ELB Health Check

209. AWS에서 리전 서비스로 간주되는 것은 무엇입니까?
- Amazon EFS
- AWS Batch

Batch는 한 리전 내 여러 가용 영역에서 배치 작업 실행을 단순화하는 리전 서비스입니다. 

EFS는 여러 AZ 내외에 데이터를 저장하는 리전 서비스입니다. Amazon EC2 인스턴스는 AZ, 리전 및 VPC 전반에 걸쳐 파일 시스템에 액세스할 수 있으며, 온프레미스 서버는 AWS Direct Connect 또는 AWS VPN을 사용하여 액세스할 수 있습니다.

[오답]
 EC2 : Amazon EC2는 시작된 특정 가용 영역에 연결되어 있으므로 "영역 서비스(AZ)"로 간주됩니다.

영역 서비스 - 리전 서비스 - 글로벌 서비스
EC2

210. 이벤트 기반 앱을 구축하고 앱 아키텍처의 구성 요소를 분리할 수 있는 AWS 서비스는?
- Amazon EventBridge (Amazon CloudWatch Events)
이벤트 중심 앱의 신속한 개발을 지원하는 서버리스 인벤트 버스 서버.

[오답]
Amazon SQS : Amazon SQS는 마이크로서비스, 분산 시스템 및 서버리스 애플리케이션을 분리하고 확장하기 위한 관리형 메시지 대기열 서비스를 제공합니다. 직접적인 통신 없이 비동기적으로 메시지를 보내고 받을 수 있도록 함으로써 애플리케이션 구성 요소의 분리를 가능하게 합니다.

Amazon SNS : 이 서비스는 애플리케이션 간 및 애플리케이션 간(예: SMS) 통신을 위한 완전 관리형 게시/구독 메시징 서비스를 제공합니다. 이를 통해 애플리케이션 구성 요소가 비동기적으로 메시지를 보내고 받을 수 있도록 함으로써 애플리케이션 구성 요소를 분리할 수 있습니다.

211. 다음 중 "리전, 인스턴스 패밀리, 크기, OS 또는 테넌시에 관계없이 EC2 인스턴스 사용량에 대한 비용을 "자동"으로 줄여주는 요금 옵션은 무엇입니까?
- Saving Plans
AWS Savings Plan은 Amazon EC2, AWS Fargate 및 AWS Lambda 사용량을 최대 72%까지 절약할 수 있는 유연한 요금 모델입니다. Savings Plans는 1년 또는 3년 동안 "일관된 사용량($/시간 단위로 측정)을 약속하는 대가"로 Amazon EC2 사용량, Fargate 및 Lambda에 대해 더 낮은 가격을 제공합니다.

212. AWS Control Tower의 주요 사용 사례는?
- AWS의 잘 설계된 다중 계정 환경을 구현하고 AWS 모범 사례를 적용하는 랜딩 존을 설정하는 쉬운 방법
모범 사례를 통해 "다중 계정 AWS 환경" 을 생성하거나 관리하려는 고객을 위한 것

213. 회사는 클라우드에서 지속적인 운영을 준비하는 동시에 새로운 새로운 워크로드를 배포, 구성 및 보호하는 데 도움이 되는 안내 경로를 찾고 있습니다. 

- Cloud Foundations
고객이 새로운 워크로드를 배포, 구성 및 보호하는 동시에 클라우드에서 지속적인 운영에 대비할 수 있도록 도와주는 가이드 경로를 제공합니다.
Cloud Foundations는 선별된 AWS 서비스, AWS 솔루션, 파트너 솔루션 및 지침을 통해 고객이 내려야 할 결정을 내릴 수 있도록 도와줍니다. 이러한 솔루션은 클라우드 기반 여정을 지원하여 프로덕션에 바로 사용할 수 있는 환경의 구축을 가속화합니다.

[오답]
AWS Trusted Advisor
AWS 모범 사례에 따라 리소스를 프로비저닝하데 도움이 되는 실시간 지침을 제공하는 온라인 도구입니다.
Trusted Advisor가 정기적으로 제공하는 권장 사항은 솔루션을 최적의 상태로 프로비저닝하는 데 도움이 됩니다. 

214. 회사는 웹사이트에 SQL 삽입, 교차 사이트 스크립팅 등 일반적인 웹 공격으로부터 웹 애플리케이션을 보호하려고 합니다. 다음 중 이 사례를 해결하는 데 사용할 수 있는 AWS 서비스는 무엇입니까?
- AWS WAF

215. 신규 AWS 고객은 최근 출시한 VPC에 최대 100개의 t3a.large EC2 인스턴스를 배포해야 하는데, 이는 기본 서비스 한도를 훨씬 초과하는 것입니다. 추가 인스턴스를 시작하려면 어떻게 해야 합니까?

- AWS 지원 센터 페이지에서 사례를 생성하고 서비스 한도 증가를 요청합니다.
->기본 서비스 한도를 훨씬 초과할 때에는 AWS지원 서비스센터에 요청해야함

[오답]
아무것도하지 않습니다.. AWS가 서비스 한도를 자동으로 늘려주므로 동시에 100개의 t3a.large EC2 인스턴스를 직접 시작할 수 있습니다. : AWS는 고객을 위해 이 작업을 수행하지 않습니다. 서비스 한도 증가 요청은 AWS에서 자동으로 수행되지 않으므로 이를 제기하는 것은 고객의 책임입니다.

216. AWS Well-Architected Framework의 안정성 원칙의 일부인 조직 변경 관리를 촉진하는 데 사용할 수 있는 AWS 서비스는 무엇입니까? (2개 선택)
- Amazon CloudWatch
- AWS CloudTrail

Amazon CloudWatch는 DevOps 엔지니어, 개발자, 사이트 안정성 엔지니어(SRE) 및 IT 관리자를 위해 구축된 모니터링 및 관찰 서비스입니다. CloudWatch는 애플리케이션을 모니터링하고, 시스템 전반의 성능 변화에 대응하고, 리소스 활용도를 최적화하고, 운영 상태에 대한 통합 보기를 얻기 위한 데이터와 실행 가능한 통찰력을 제공합니다.

CloudTrail은 AWS Management Console, AWS SDK, 명령줄 도구 및 기타 AWS 서비스를 통해 수행된 작업을 포함하여 AWS 계정 활동의 이벤트 기록을 제공합니다.

[오답]
AWS Trusted Advisor : AWS Trusted Advisor는 비용 최적화, 보안, 내결함성, 서비스 제한 및 성능 개선에 대한 AWS "모범 사례"에 따라 "리소스를 프로비저닝"하는 데 도움이 되는 실시간 지침을 제공하는 온라인 도구입니다.

217. EC2 CPU 사용률 임계값이 위반될 때 알려주는 경보를 생성할 수 있는 서비스는 무엇입니까?
- CloudWatch

218. 회사는 Amazon S3 버킷을 보호하고 규정 준수 요구 사항을 충족하기 위한 공개 객체를 가질 수 없도록 해야 합니다.
이를 쉽게 수행하려면 어떤 S3 기능을 사용해야 합니까?
- Block Public Access

[오답]
Security Groups : Amazon S3에는 보안 그룹이 없습니다.

219. 고객은 Amazon S3를 사용하여 게임 캐릭터의 이미지를 저장하고 있습니다. 이미지는 자주 액세스하지 않지만 플레이어가 가끔 검색을 합니다. 플레이어가 이러한 이미지를 저장하면 플레이어의 컴퓨터에도 저장됩니다. 이미지는 현재 S3 Standard 스토리지 클래스에 저장되어 있습니다. 다음 중 스토리지 비용을 최적화하기 위해 권장하는 옵션은 무엇입니까?

정답
고객이 이미지를 S3 Standard - Infrequent Acces에 직접 업로드하도록 합니다.

알 수 없거나 액세스 패턴이 변경되는 데이터를 위한 S3 Intelligent-Teiring; S3 Standard-Infrequest Acess 및 S3 One Zone- IA는 수명이 길지만 자주 액세스하지 않는 데이터를 위한 것입니다.
이미지는 거의 액세스되지 않으므로(액세스한 후에는 사용자 컴퓨터에 저장됨) S3 Standard-IA 스토리지 클래스를 사용하는 것이 가장 좋은 스토리지 유형입니다.

[오답]
고객이 이미지를 업로드한 후 S3 Standard - Infrequent Access로 이미지를 이동하는 수명 주기 정책을 추가합니다. : 객체를 S3 Standard-IA에 직접 저장할 수 있으므로 필요하지 않습니다.

220. 고성능 모바일 앱의 이점을 위해 5G 네트워크와 AWS 컴퓨팅 및 스토리지 서비스를 결합한 서비스는 무엇입니까?
- AWS Wavelength

221. 백엔드 구성 요소용 서버를 프로비저닝, 유지 및 관리할 필요가 없는 AWS 서버리스 플랫폼의 일부인 서비스는 무엇입니까? (2개 선택)
- Lambda
- Amazon API Gateway

Lambda, AWS Fargate는 서버리스 컴퓨팅에 사용할 수 있는 서비스입니다.
API프록시의 경우 API Gateway 서비스의 기능을 활용할 수 있습니다.

API Gateway - 서버리스 플랫폼..

222. AWS Cost and cost report
 - Amazon Athena, Amazon Redshift 및 AWS QuickSight에 비용 및 사용 정보를 로드할 수 있습니다.
 - 해당 "임계값이 초과될 때 경고하는 사용자 지정 비용 및 사용 예산을 설정"할 수 있습니다. AWS 비용 및 사용량에 대한 세부적인 데이터를 제공합니다.

사용자 지정 보고서를 빠르게 생성할 수 있는 직관적인 인터페이스를 통해 시간 경과에 따른 AWS 비용 및 사용량을 시각화, 이해 및 관리하는 데 도움이 됩니다. : 이는 AWS Cost Explorer를 나타냅니다. 이 서비스는 일별 또는 월별 AWS 비용 및 사용량을 시각화, 이해 및 관리하는 데 도움이 되는 대시보드를 제공합니다.

시각화 -> Cost Exploerer

 월별 AWS 지출 상태를 볼 수 있는 대시보드를 제공하고, AWS 비용 및 사용량을 더 자세히 파악하는 데 도움이 되는 다양한 기타 비용 관리 제품에 대한 액세스를 제공합니다. : 이는 AWS 비용 및 사용량에 대한 포괄적인 정보에 빠르게 액세스할 수 있는 AWS 비용 관리 대시보드를 의미합니다.
 "대시보드" : 대시보드

223. Amazon ElastiCache에서는 어떤 유형의 캐싱 솔루션을 사용할 수 있습니까? (2개 선택)
- Redis용 Amazon ElastiCache
- Memcached용 Amazon ElastiCache

224. 다음 중 AWS Global Accelerator를 사용하면 어떤 이점이 있습니까?
- AWS에서 호스팅되는 애플리케이션 액세스 지연 시간 감소

225. 다음 중 Docker 이미지를 저장하고 Docker 컨테이너를 간단하고 비용 효율적인 방식으로 조정할 수 있는 서비스는 무엇입니까? (2개 선택)
- ECR
- ECS

Elastic Container Service(ECS) : Dokcer 컨테이너를 지원하고 AWS에서 컨테이너화된 앱을 쉽게 실행하고 확장할 수 있게 해주는 확장성이 뛰어난 고성능 컨테이너 

Elaistc Cotrainer Registry(ECR) : 개발자가 Docker 컨테이너 이미지를 쉽게 저장, 관리 및 배포할 수 있게 해주는.

226. "중앙 허브"를 통해 VPC와 온프레미스 네트워크를 연결하는 서비스는 무엇입니까
- AWS Transit Gateway
중앙 허브를 통해 VPC와 온프레미스 네트워크를 연결합니다.

227. 고객은 지정된 테크니컬 어카운트 관리가 포함된 최고의 AWS support plan을 선택할 계획입니다. 그들은 다음 중 무엇을 선택해야 합니까?
- Enterprise support plan

228. AWS Business Support plan을 사용하면 고객이 어떤 기능에 액세스할 수 있습니까?
- 아키텍처 지원

229. Amazon Linux EC2 인스턴스가 1시간 30분 동안 실행되고 있습니다. AWS는 사용량에 대해 어떻게 요금을 청구합니까?
- 초당 과금 방식에 따라 1시간 30분에 대한 요금 청구

EC2는 초당 과금 방식.. 메모..

230. 다음 중 사용 가능한 "모든 AWS 서비스의 일반 상태를 표시"하고 서비스에 가용성 문제가 있는지 알려주는 서비스는 무엇입니까?
...
232. AWS 클라우드의 "모든 리전에서 모든 AWS 서비스의 일반 상태" 및 가용성에 대한 최신 정보를 게시하는 AWS 서비스는 무엇입니까?
- AWS Health Dashboard - service health

233. AWS 공동 책임 모델에서 다음 중 AWS Lambda와 관련하여 고객의 책임은 무엇입니까?
- AWS Lambda 함수 버전 유지

234. 고성능 컴퓨팅 앱에는 여러 EC2인스턴스에 대한 "중앙 집중식 Windows 파일 서버"로 사용할 수 있는 AWS 스토리지 서비스가 필요합니다.
- Amazon FSx
 NetApp ONTAP, OpenZFS, Windows File Server, Lustre 중 선택 가능
Amazon FSx for Windows File Server : Windows Server에 구축되는 완전관리형 파일 스토리지

235. 개발자는 Docker 컨테이너에 애플리케이션을 설치해야 합니다. 다음 중 컨테이너를 수동으로 관리할 필요가 없는 서비스는 무엇입니까?

- AWS Fargate
는 컨테이너 전용 서버리스 컴퓨팅 엔진. 서버를 프로비저닝하고 관리해준다. 
Fargate를 사용하면 애플리케이션 콘텐츠, 네트워킹, 스토리지 및 확장 요구 사항을 정의할 수 있습니다. 프로비저닝, 패치 적용, 클러스터 용량 관리 또는 인프라 관리가 필요하지 않습니다.

236. 다음 중 "사전 정의된 AWS 리소스 포트폴리오"에 대한 액세스를 제한할 수 있는 서비스는 무엇입니까?
- AWS Service Catalog 

AWS Service Catalog에서 "포트폴리오"는 조직에서 사전 승인되어 사용자에게 제공되는 IT 서비스 모음입니다. 포트폴리오에는 하나 이상의 제품이 포함될 수 있으며, 각 제품은 특정 서비스나 기능을 제공하기 위해 함께 작동하도록 사전 구성 및 테스트된 AWS 리소스 세트입니다.
다양한 유형의 최종 사용자에 맞춰진 다양한 제품 및 액세스 권한을 사용하여 여러 포트폴리오를 생성할 수 있습니다. 예를 들어, 개발 팀의 포트폴리오에는 영업 및 마케팅 팀의 포트폴리오와 다른 제품이 포함될 가능성이 높습니다. 단일 제품은 각각 고유한 액세스 및 "프로비저닝 정책 세트"를 포함하는 여러 포트폴리오에 게시될 수 있습니다.

[오답]
AWS Config : AWS Config는 주로 AWS 리소스를 지속적으로 모니터링하고 변경 사항을 기록하며 구성 기록, 변경 알림 및 규정 준수 보고를 제공하는 데 사용됩니다. AWS Config를 사용하여 조직 전체에서 제어되고 표준화된 방식으로 프로비저닝 및 배포할 수 있는 IT 서비스 카탈로그를 생성하고 관리할 수는 없습니다.

DynamoDB는 SLA에 따라 내구성 측면에서 99.999999999%를 제공합니다. : Amazon S3와 달리 내구성이 99.999999999%를 제공하지 않습니다.

237.  AWS 비용을 시간별로 분류한 자세한 보고서
- AWS Cost and Usage Reposrt (AWS CUR)

238. 추가 고스트웨어를 사용하지 않고 두 개의 VPC같 프라이빗 연결을 설정해야 한다.
- VPC Peering
프라이빗 IP주소를 사용하여 두 VPC간 트래픽을 라우팅 할 수 있게 해주는 두 VPC간 네트워킹 연결.

239. 여러 AWS 서비스를 서버리스 워크플로로 조정하려는 경우 비용 효율적인 서비스는?
시각적 워크플로를 사용하여 분산 앱과 마이크로서비스의 구성 요소를 조정

- AWS Step Functions
서버리스 오케스트레이션을 제공 
오케스트레이션이란 :워크플로를 여러 단계로 나누고, 흐름 논리를 추가하고, 단계 간 입력과 출력을 추적하여 워크를로를 중앙에서 관리한다.

Step Functions를 사용하면 비즈니스 로직과 독립적으로 애플리케이션의 워크플로를 정의하고 관리할 수 있으므로 애플리케이션 개발이 더 빠르고 직관적입니다. 하나를 변경해도 다른 항목에는 영향을 주지 않습니다. 여러 지점 간 통합을 관리, 모니터링 및 유지하는 데 어려움을 겪을 필요 없이 한 곳에서 워크플로를 쉽게 업데이트하고 수정할 수 있습니다. Step Functions는 함수와 컨테이너를 과도한 코드로부터 해방시켜 애플리케이션을 더 빠르게 작성하고 복원력을 강화하며 유지 관리를 더 쉽게 해줍니다.

240. 다음 중 Amazon EC2 컨버터블 예약 인스턴스의 특징은 무엇입니까? (2개 선택)
- 교환으로 인해 동일하거나 더 큰 가치의 예약 인스턴스가 생성되는 한 RI의 속성을 변경할 수 있습니다.
- 인스턴스 제품군, 운영 체제, 테넌시 및 결제 옵션 변경을 허용합니다.

[오답]
하루, 한 주 또는 한 달의 일부만 필요한 예측 가능한 반복 일정에 용량 예약을 일치시킬 수 있습니다.
: 이는 컨버터블 RI가 아닌 예약 RI에 대한 설명입니다.

241. 다음 중 여러 전문 실무 영역에서 유료 참여를 통해 엔터프라이즈 클라우드 채택과 관련된 특정 비즈니스 성과를 달성하는 데 도움이 되는 서비스 모음을 공유하는 채널은 무엇입니까?
- AWS Professional Services
 다양한 서비스를 공유합니다. 

[오답]
AWS Enterprise Support : AWS Enterprise Support는 환경 상태를 자동으로 관리하기 위한 고품질 엔지니어, 도구 및 기술의 연중무휴 기술 지원, 애플리케이션 및 사용 사례의 맥락에서 제공되는 컨설팅 아키텍처 지침, 지정된 기술 계정 관리자( TAM)을 통해 사전 예방적/예방적 프로그램과 AWS 주제 전문가에 대한 액세스를 조정합니다.

242. 결제 및 계정 문의를 지원하고 결제 및 계정 모범 사례를 구현하기 위해 협력하는 컨시어지 지원 팀이 포함된 AWS support plan은 무엇입니까?
- Enterprise support plan : 컨시어지 지원 팀 포함은 Business는 없고 Enterprise부터.

243. S3 버킷에서 자주 액세스하지 않는 데이터를 보다 비용 효율적인 스토리지 클래스로 자동 전송하려면 ?
- Amazon S3 수명주기 정책
S3의 수명 주기 정책을 사용하면 자주 액세스하지 않는 데이터를 S3-IA 또는 Glacier와 같은 보다 비용 효율적인 스토리지 클래스로 자동 이동할 수 있습니다.
Amazon S3의 수명 주기 구성을 사용하면 버킷에 있는 객체의 수명 주기 관리를 지정할 수 있습니다. 구성은 하나 이상의 규칙 세트로, 각 규칙은 Amazon S3가 객체 그룹에 적용할 작업을 정의합니다. 이러한 조치는 다음과 같이 분류될 수 있습니다.
전환 작업 – 객체가 다른 스토리지 클래스로 전환되는 시기를 정의합니다. 예를 들어 생성 후 30일 후에 객체를 STANDARD_IA(IA, 자주 액세스하지 않는 경우) 스토리지 클래스로 전환하거나 생성 후 1년 후에 GLACIER 스토리지 클래스에 객체를 보관하도록 선택할 수 있습니다.
만료 작업 – 객체가 만료되는 시기를 지정합니다. 그러면 Amazon S3가 사용자를 대신하여 만료된 객체를 삭제합니다.

244. CloudFormation템플릿을 통해 AWS 주요 워크로드에 대한 자동 참조 배포를 제공하는 AWS 서비스는?
- AWS Quick Starts
